...
function determineDirection(
  ...
  // Version comparisons - newer is better (semantic, not lexicographic)
  if (field.toLowerCase().includes('version')) {
    const beforeStr = String(before || '');
    const afterStr = String(after || '');
    const bp = beforeStr.split(/[.-]/).map(p => parseInt(p, 10) || 0);
    const ap = afterStr.split(/[.-]/).map(p => parseInt(p, 10) || 0);
    for (let i = 0; i < Math.max(bp.length, ap.length); i++) {
      if ((ap[i] || 0) > (bp[i] || 0)) return 'improvement';
      if ((ap[i] || 0) < (bp[i] || 0)) return 'regression';
    }
    return 'neutral';
  }
