...
export class GeneralSettingsParser extends BaseJSONParser<GeneralSettingsResult> {
  ...
  private checkDisabledFeatures(
    ...
    if (data.aiDrivenAnalyticsSettings) {
      ...
      if ('enabled' in ai) {
        // DSS 14+: single toggle for Dataiku AI Services
        if (ai.enabled === false)
          add(
            'Dataiku AI Services',
        ...
        // Old format (DSS 12.x): granular toggles indicate outdated DSS
        add(
          'Dataiku AI Services',
          'DSS version too old — upgrade to DSS 14+ for unified Dataiku AI Services settings',
          'https://doc.dataiku.com/dss/latest/ai-assistants/index.html'
        );
      }
    }
  ...
  private parseCgroupsSettings(data: GeneralSettingsData): CgroupSettings {
    ...
    if (cgroups.cgroups && cgroups.cgroups.length > 0) {
      ...
      if (mainCgroup.limits && mainCgroup.limits.length > 0) {
        const memLimit = mainCgroup.limits.find(l => l.key === 'memory.max');
        if (memLimit?.value) {
          cgroupSettings['Memory Limit'] = memLimit.value;
        }
      }
