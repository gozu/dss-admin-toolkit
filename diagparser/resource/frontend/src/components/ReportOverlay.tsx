      ...
      <div className="report-slides-container">
        ...
        <div className={`report-slide report-slide-hero${currentSlide === 0 ? ' active' : ''}`} data-slide-index={0}>
          <div className="report-title-center">
            ...
            <div className="report-title-company" contentEditable suppressContentEditableWarning>{company}</div>
            <div className="report-title-divider" />
            <div className="report-title-sub" contentEditable suppressContentEditableWarning>Quarterly Health Check</div>
            <div className="report-title-meta" contentEditable suppressContentEditableWarning>
        ...
        <div className={`report-slide${currentSlide === 1 ? ' active' : ''}`} data-slide-index={1}>
          <div className="report-slide-header">
            ...
            <div className="report-slide-title" contentEditable suppressContentEditableWarning>Executive Summary</div>
          </div>
          <div className="report-exec-layout">
            <div className="report-exec-score">
              <div className={`report-health-score ${healthScore.status}`} contentEditable suppressContentEditableWarning>
                {healthScore.overall}
              </div>
              <div className="report-metric-label" contentEditable suppressContentEditableWarning style={{ textAlign: 'center' }}>Health Score</div>
              <div className={`report-badge report-badge-${healthScore.status === 'healthy' ? 'nice' : healthScore.status === 'warning' ? 'important' : 'critical'}`} contentEditable suppressContentEditableWarning>
                {healthScore.status}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '1rem', width: '100%' }}>
                {healthScore.categories.map((cat, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.4rem 0.75rem', background: 'rgba(254,254,249,0.05)', borderRadius: '0.4rem', fontSize: '0.85rem' }}>
                    <span contentEditable suppressContentEditableWarning style={{ color: 'rgba(254,254,249,0.7)', textTransform: 'capitalize' }}>{cat.category.replace(/_/g, ' ')}</span>
                    <span contentEditable suppressContentEditableWarning style={{ fontFamily: "'DM Mono', monospace", fontWeight: 600, color: cat.score < 50 ? '#ef4444' : cat.score < 80 ? '#EDAB4F' : '#3EDAB2' }}>{cat.score}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <DataSlide index={3} slideNum="03" active={currentSlide === 3} title="Projects Overview"
          ...
          leftExtras={slides?.projects?.highlights?.length ? (
            <div className="report-extras-list">
              {slides.projects.highlights.map((h, i) => (
                <div key={i} className="report-extras-item" contentEditable suppressContentEditableWarning>• {h}</div>
              ))}
            </div>
          ) : undefined}
        />
        ...
        <DataSlide index={4} slideNum="04" active={currentSlide === 4} title="Project Footprint"
          ...
          leftExtras={slides?.project_footprint?.risks?.length ? (
            <div className="report-extras-badges" style={{ marginTop: '1rem' }}>
              {slides.project_footprint.risks.map((r, i) => (
                <span key={i} className="report-badge report-badge-important" contentEditable suppressContentEditableWarning>{r}</span>
              ))}
            </div>
          ) : undefined}
        />
        ...
        <DataSlide index={6} slideNum="06" active={currentSlide === 6} title="Code Environment Health"
          ...
          leftExtras={slides?.code_env_health?.upgrade_paths?.length ? (
            <div className="report-extras-list" style={{ marginTop: '1rem' }}>
              {slides.code_env_health.upgrade_paths.map((u, i) => (
                <div key={i} className="report-extras-item" contentEditable suppressContentEditableWarning style={{ fontSize: '1.25rem' }}>→ {u}</div>
              ))}
            </div>
          ) : undefined}
        />
          ...
          extras={slides?.filesystem?.warnings?.length ? (
            <div className="report-extras-list" style={{ marginTop: '1rem' }}>
              {slides.filesystem.warnings.map((w, i) => (
                <div key={i} className="report-extras-item" contentEditable suppressContentEditableWarning style={{ borderLeftColor: '#ef4444', fontSize: '1.15rem' }}>⚠ {w}</div>
              ))}
            </div>
          ) : undefined}
        />
        ...
        <DataSlide index={8} slideNum="08" active={currentSlide === 8} title="Memory & JVM"
          ...
          leftExtras={slides?.memory?.tuning_recs?.length ? (
            <div className="report-extras-list" style={{ marginTop: '1rem' }}>
              {slides.memory.tuning_recs.map((r, i) => (
                <div key={i} className="report-extras-item" contentEditable suppressContentEditableWarning>• {r}</div>
              ))}
            </div>
          ) : undefined}
        />
        ...
        <DataSlide index={9} slideNum="09" active={currentSlide === 9} title="Connections"
          ...
          leftExtras={(() => {
            const counts = parsedData.connectionCounts || {};
            const remaining = Object.entries(counts).sort(([,a],[,b]) => b - a).slice(2, 8);
            if (!remaining.length) return undefined;
            return (
              <div className="report-extras-list" style={{ marginTop: '1rem' }}>
                {remaining.map(([type, count], i) => (
                  <div key={i} className="report-extras-item" contentEditable suppressContentEditableWarning>{type}: {count}</div>
                ))}
              </div>
            );
          })()}
        ...
        <DataSlide index={10} slideNum="10" active={currentSlide === 10} title="Issues & Risks"
          ...
          leftExtras={(() => {
            const disabled = parsedData.disabledFeatures || {};
            const items = Object.entries(disabled).slice(0, 6);
            if (!items.length) return undefined;
            return (
              <div className="report-extras-list" style={{ marginTop: '1rem' }}>
                {items.map(([key, val], i) => (
                  <div key={i} className="report-extras-item" contentEditable suppressContentEditableWarning style={{ borderLeftColor: '#ef4444' }}>
        ...
        <DataSlide index={11} slideNum="11" active={currentSlide === 11} title="Users & Activity"
          ...
          leftExtras={(() => {
            ...
            users.forEach(u => { const p = u.userProfile || 'Unknown'; profileCounts[p] = (profileCounts[p] || 0) + 1; });
            const sorted = Object.entries(profileCounts).sort(([,a],[,b]) => b - a).slice(0, 6);
            if (!sorted.length) return undefined;
            return (
              <div className="report-extras-list" style={{ marginTop: '1rem' }}>
                {sorted.map(([profile, count], i) => (
                  <div key={i} className="report-extras-item" contentEditable suppressContentEditableWarning>{profile}: {count}</div>
                ))}
              </div>
            );
          })()}
          ...
          extras={slides?.logs?.patterns?.length ? (
            <div className="report-extras-list" style={{ marginTop: '1rem' }}>
              {slides.logs.patterns.slice(0, 5).map((p, i) => (
                <div key={i} className="report-extras-item" contentEditable suppressContentEditableWarning style={{ fontFamily: "'DM Mono', monospace", fontSize: '1.1rem', color: 'rgba(254, 254, 249, 0.7)' }}>{p}</div>
              ))}
            </div>
          ) : undefined}
        />
        ...
        <div className={`report-slide${currentSlide === 16 ? ' active' : ''}`} data-slide-index={16}>
          <div className="report-slide-header">
            ...
            <div>
              <div className="report-slide-title" contentEditable suppressContentEditableWarning>Action Plan</div>
              <div className="report-slide-subtitle" contentEditable suppressContentEditableWarning>Prioritized roadmap for the next quarter</div>
            </div>
          </div>
          <div className="report-action-list">
            {(slides?.action_plan?.priorities || []).map((p, i) => (
              <div key={i} className="report-action-row">
                ...
                <span className="action-timeline" contentEditable suppressContentEditableWarning>{p.timeline}</span>
                <span className={`report-badge report-badge-effort-${p.effort || 'medium'}`} contentEditable suppressContentEditableWarning>{p.effort || 'medium'}</span>
              </div>
            ))}
        ...
        <div className={`report-slide report-slide-hero${currentSlide === 17 ? ' active' : ''}`} data-slide-index={17}>
          <div className="report-title-center">
            <img src={dkulogo} alt="Dataiku" id="dku-logo-closing" className="report-title-logo" style={{ width: 130, height: 130 }} />
            <div className="report-title-divider" />
            <div className="report-title-company" contentEditable suppressContentEditableWarning style={{ fontSize: '2.5rem' }}>{company}</div>
            <div className="report-title-meta" contentEditable suppressContentEditableWarning>{date}</div>
          </div>
        </div>
      </div>
...
function DataSlide({ index, slideNum, active, title, metrics, narrative, extras, leftExtras }: {
  index: number; slideNum: string; active: boolean; title: string;
  metrics: MetricItem[]; narrative?: string; extras?: React.ReactNode; leftExtras?: React.ReactNode;
}) {
  return (
    <div className={`report-slide${active ? ' active' : ''}`} data-slide-index={index}>
      <div className="report-slide-header">
        <div className="report-slide-number">{slideNum}</div>
        <div className="report-slide-title" contentEditable suppressContentEditableWarning>{title}</div>
      </div>
          ...
          <div className="report-metrics-grid">
            {metrics.map((m, i) => (
              <div key={i} className="report-metric">
                <div className="report-metric-value" contentEditable suppressContentEditableWarning style={m.color ? { color: `var(--${m.color})` } : undefined}>{fmt2dp(m.value)}</div>
                <div className="report-metric-label" contentEditable suppressContentEditableWarning>{m.label}</div>
              </div>
            ))}
          </div>
          {leftExtras}
...
function RecSlide({ index, slideNum, active, title, badgeClass, badgeText, numberClass, items, layout }: {
  index: number; slideNum: string; active: boolean; title: string;
  badgeClass: string; badgeText: string; numberClass: string;
  items: Array<{ title: string; description: string; impact: string }>;
  layout?: 'single-col' | 'full-span-last';
}) {
  return (
    <div className={`report-slide${active ? ' active' : ''}`} data-slide-index={index}>
      <div className="report-slide-header">
        <div className="report-slide-number">{slideNum}</div>
        <div className="report-slide-title" contentEditable suppressContentEditableWarning>{title}</div>
        <span className={`report-badge ${badgeClass}`} contentEditable suppressContentEditableWarning>{badgeText}</span>
      </div>
      <div className={`report-rec-list${layout === 'single-col' ? ' single-col' : ''}`}>
        {items.map((item, i) => (
          <div key={i} className={`report-rec-card${layout === 'full-span-last' && i === items.length - 1 && items.length % 2 === 1 ? ' full-span' : ''}`}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {item.impact && (
                <div className="report-rec-impact" contentEditable suppressContentEditableWarning>
