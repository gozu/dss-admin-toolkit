...
import type { ParsedData, DeltaDirection, DeltaSeverity } from '../../types';
...
function SystemField({ label, beforeValue, afterValue, icon, highlight = false, delay = 0, severity, direction }: SystemFieldProps) {
  ...
  const changedBg = severity === 'critical'
  ...
  return (
    <motion.div
      className={`p-4 rounded-lg ${changed ? changedBg : 'bg-[var(--bg-glass)]'}`}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            {changed && <DeltaBadge changeType={changeType} direction={direction} severity={severity} />}
          </div>
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        {(beforeData.dssVersion || afterData.dssVersion) && (() => {
          const dssChanged = beforeData.dssVersion !== afterData.dssVersion;
          const dssVersionCmp = compareVersions(afterData.dssVersion, beforeData.dssVersion);
          const dssDirection: DeltaDirection = dssVersionCmp > 0 ? 'improvement' : dssVersionCmp < 0 ? 'regression' : 'neutral';
          const dssSeverity: DeltaSeverity = dssChanged && dssDirection === 'regression' ? 'critical' : 'warning';
          const dssBg = dssChanged
            ? dssSeverity === 'critical'
              ? 'bg-[var(--status-critical-bg)] border border-[var(--status-critical-border)]'
              : 'bg-[var(--status-warning-bg)] border border-[var(--status-warning-border)]'
            : 'bg-[var(--bg-glass)]';
          return (
          <motion.div
            className={`p-4 rounded-lg ${dssBg}`}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {beforeData.dssVersion !== afterData.dssVersion && (
                    <DeltaBadge changeType="modified" direction={dssDirection} severity={dssSeverity} />
                  )}
                </div>
                <div className="flex flex-col gap-1">
        ...
        {(beforeData.pythonVersion || afterData.pythonVersion) && (() => {
          const pyChanged = beforeData.pythonVersion !== afterData.pythonVersion;
          const pyVersionCmp = compareVersions(afterData.pythonVersion, beforeData.pythonVersion);
          const pyDirection: DeltaDirection = pyVersionCmp > 0 ? 'improvement' : pyVersionCmp < 0 ? 'regression' : 'neutral';
          const pySeverity: DeltaSeverity = pyChanged && pyDirection === 'regression' ? 'critical' : 'warning';
          const pyBg = pyChanged
            ? pySeverity === 'critical'
              ? 'bg-[var(--status-critical-bg)] border border-[var(--status-critical-border)]'
              : 'bg-[var(--status-warning-bg)] border border-[var(--status-warning-border)]'
            : 'bg-[var(--bg-glass)]';
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {beforeData.pythonVersion !== afterData.pythonVersion && (
                    <DeltaBadge changeType="modified" direction={pyDirection} severity={pySeverity} />
                  )}
                </div>
