...
import { AgenticDetailsModal } from './AgenticDetailsModal';
...
export function ResultsView({ onBack }: ResultsViewProps) {
  ...
  const agenticModal = useModal();

  const [viewingFile, setViewingFile] = useState<{ name: string; content: string } | null>(null);
  const [viewingProject, setViewingProject] = useState<Project | null>(null);
  const [viewingAgenticProject, setViewingAgenticProject] = useState<Project | null>(null);
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => setKeyBuf(b => (b + e.key).slice(-6));
  ...
  // Agentic details handler
  const handleViewAgentic = useCallback((project: Project) => {
    setViewingAgenticProject(project);
    agenticModal.open();
  }, [agenticModal]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--bg-void)]">
      ...
      <main className="flex-1 py-4">
        <Container>
          ...
          <CardGrid>
            ...
            {/* Unified Projects table (governance + footprint) */}
            <div className="col-span-full">
              <ProjectsTable onViewPermissions={handleViewPermissions} onViewAgentic={handleViewAgentic} />
            </div>

            {/* Filesystem | Connections | Memory — equal thirds */}
            <div className="col-span-full grid grid-cols-1 md:grid-cols-3 gap-4">
              <div><FilesystemChart /></div>
              <div><ConnectionsChart /></div>
              <div><MemoryChart /></div>
            </div>
            ...
            {/* Full-width Connection Usage card */}
            <ConnectionUsageCard />

            {/* LLM Model Audit — full width */}
            {parsedData.llmAudit && (
              <div className="col-span-full">
                <LlmAuditPage />
              </div>
      ...
      {/* Agentic Details Modal */}
      <AgenticDetailsModal
        isOpen={agenticModal.isOpen}
        onClose={agenticModal.close}
        project={viewingAgenticProject}
      />
    </div>
  );
}
