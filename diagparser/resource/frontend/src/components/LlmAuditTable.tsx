...
// not_applicable rows are filtered out in the parser, so it never reaches this table.
...
function statusBadgeClass(status: DisplayStatus): string {
  switch (status) {
    case 'ripoff':
      return 'bg-[var(--neon-red)]/20 text-[var(--neon-red)] border border-[var(--neon-red)]/40';
    case 'obsolete':
...
export function LlmAuditTable() {
  ...
  const counts = useMemo(() => {
    const c: Record<DisplayStatus, number> = { ripoff: 0, obsolete: 0, current: 0, unknown: 0 };
  ...
  return (
    <div className="space-y-4">
      <SummaryCard
        ...
        lookupLoadedAt={audit?.lookupLoadedAt}
        loading={isLoading}
      />
      ...
      >
        ...
        {isLoading && (
          <div className="px-4 py-3 border-b border-[var(--border-glass)]">
            <div className="flex items-center justify-between text-xs text-[var(--text-secondary)]">
              <span>{loading?.message || 'Fetching LiteLLM catalog...'}</span>
        ...
        {audit?.lookupError && (
          <div className="px-4 py-3 border-b border-[var(--border-glass)] text-xs text-[var(--neon-amber)]">
            Could not fetch LiteLLM pricing catalog: {audit.lookupError}. Model statuses are unknown.
          </div>
        )}

        {grouped.length === 0 && !isLoading ? (
          <div className="p-6 text-sm text-[var(--text-secondary)]">
            No LLM Mesh connections found in this diagnostic ZIP.
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-[var(--border-glass)]">
                {sorted.map((r) => (
                  <tr key={r.llmId} className="hover:bg-[var(--bg-glass-hover)] transition-colors">
                    ...
                    <td className="px-4 py-3 align-top">
                      <div className="text-sm text-[var(--text-primary)]">{r.friendlyNameShort}</div>
                    ...
                    <td className="px-4 py-3 align-top text-right">
                      {r.projectsUsing > 0 ? (
                        ...
                        <span
                          ...
                          title="No scanned asset references this LLM"
                        >
                          0 projects
                        </span>
                      )}
      ...
      {modalLlm && <ProjectsModal row={modalLlm} onClose={() => setModalLlm(null)} />}
    </div>
  );
}
    ...
    >
      ...
      >
        ...
        <div className="px-4 py-2 border-t border-[var(--border-glass)] text-[11px] text-[var(--text-tertiary)]">
          References found in recipes, knowledge banks, agents, and agent tools. Notebooks and
          dynamically-built llmIds are not scanned.
        </div>
      </div>
    </div>
  );
...
function SummaryCard({
  ...
  lookupLoadedAt: string | null | undefined;
  loading: boolean;
}) {
  const tiles: Array<{ label: string; value: number; sub?: string; color: string }> = [
    { label: 'LLMs', value: total, color: 'text-[var(--text-primary)]' },
    {
      label: 'Ripoff',
      value: counts.ripoff,
    ...
    { label: 'Current', value: counts.current, color: 'text-[var(--neon-green)]' },
    {
      label: 'Unknown',
      ...
      <div className="px-4 py-3 border-b border-[var(--border-glass)] flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-[var(--text-tertiary)] mt-0.5">
            Connections scanned against the LiteLLM pricing catalog. Pricing fetched{' '}
            {formatRelative(lookupLoadedAt)}.
          </p>
        </div>
        {loading && (
