...
function UploadSlot({ slot, file, isProcessing, onFileSelect, onClear }: UploadSlotProps) {
  const [isDragging, setIsDragging] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const label = slot === 'before' ? 'Before' : 'After';
  const description = slot === 'before' ? 'Older diagnostic' : 'Newer diagnostic';
...
export function ComparisonUpload() {
  ...
  const handleFileSelect = useCallback(async (slot: 'before' | 'after', file: File) => {
    ...
    try {
      ...
      // Parse the files synchronously to get ParsedData
  ...
  }, [processFile, parseFilesSync, setComparisonFile, setComparisonProcessing, setError]);
  ...
  const handleCompare = useCallback(() => {
    ...
    const beforeDate = parseDiagDate(beforeFile.filename);
    const afterDate = parseDiagDate(afterFile.filename);
