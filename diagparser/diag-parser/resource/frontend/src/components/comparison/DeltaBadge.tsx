import type { ChangeType, DeltaDirection, DeltaSeverity } from '../../types';
...
export function DeltaBadge({ changeType, direction, severity, size = 'sm' }: DeltaBadgeProps) {
  ...
  // Fallback: critical severity without direction still gets red styling
  if (changeType === 'modified' && severity === 'critical' && !direction) {
    finalClass = 'bg-[var(--status-critical-bg)] text-[var(--neon-red)] border-[var(--status-critical-border)]';
  }
