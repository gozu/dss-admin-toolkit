...
const CATEGORY_WEIGHTS: Record<HealthCategory, number> = {
  version: 0.25,   // 25% - Python, Spark versions
  system: 0.30,    // 30% - Memory, disk, resources
  config: 0.30,    // 30% - Disabled features, Java memory settings
...
export function calculateHealthScore(parsedData: ParsedData): HealthScore {
    ...
    // Check memory provisioning - calculate values directly from parsed data
    let memoryProvisioningScore = 100;
    ...
    if (cgroupLimit > 0) {
      ...
      if (cgroupOverageGB > 0) {
        memoryProvisioningScore = Math.min(memoryProvisioningScore, 70);
        memoryIssues.push({
          id: 'cgroup-exceeds-recommended',
          category: 'config',
          severity: 'warning',
          title: `CGroup limit exceeds recommended max by ${cgroupOverageGB}GB`,
          description: 'CGroup memory limit is higher than the recommended maximum for this VM size.',
          recommendation: 'Consider reducing the cgroup limit to the recommended value.',
        });
      }

      // Check 2: JEK allocation within cgroup
      const availableGB = cgroupLimit - (jekGB * maxJobs);
      if (availableGB < 0) {
        memoryProvisioningScore = Math.min(memoryProvisioningScore, 20);
        memoryIssues.push({
          id: 'jek-overprovisioned',
          category: 'config',
          severity: 'critical',
          title: `JEK memory over-provisioned by ${Math.abs(availableGB)}GB`,
          description: 'JEK allocation exceeds cgroup limit. OOM kills likely under full load.',
          recommendation: 'Reduce JEK heap, max running activities/jobs, or increase cgroup limit.',
        });
      } else if (availableGB < 16) {
        memoryProvisioningScore = Math.min(memoryProvisioningScore, 60);
        memoryIssues.push({
          id: 'memory-low-headroom',
          category: 'config',
          severity: 'warning',
          title: `Low memory headroom (${availableGB}GB available in cgroup)`,
          description: 'Only ' + availableGB + 'GB available in cgroup. Recommend at least 16GB buffer.',
        });
      }
    }

    const configScore = (disabledResult.score * 0.35) + (javaMemoryResult.score * 0.35) + (memoryProvisioningScore * 0.30);
    const configIssues: HealthIssue[] = [...javaMemoryResult.issues, ...memoryIssues];
