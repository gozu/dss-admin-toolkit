...
CREATE TABLE runs (
    ...
    coverage_status     TEXT NOT NULL DEFAULT 'complete',  -- 'complete' | 'partial' | 'failed'
...
Per-section completion status for each run. Critical for safe auto-resolution — if the `projects` API timed out, we must not auto-resolve project-based issues.

```sql
CREATE TABLE run_sections (
    run_id          INTEGER NOT NULL REFERENCES runs(run_id),
    section_key     TEXT NOT NULL,             -- 'overview' | 'users' | 'projects' | 'code_envs' | 'project_footprint' | 'scenarios' | 'outreach_data'
...
Auto-resolution for a campaign is only safe when **all** its required sections completed successfully:
...
| `auto_scenario` | `scenario` | project_key:scenario_id | `{"name":"Daily Load","type":"temporal","triggerCount":2}` |
| `scenario_frequency` | `scenario` | project_key:scenario_id | `{"minTriggerMinutes": 5, "triggerType":"temporal"}` |
| `scenario_failing` | `scenario` | project_key:scenario_id | `{"lastOutcome":"FAILED","consecutiveFailures":3}` |
...
CREATE TABLE issues (
    ...
    resolved_at         TEXT,
    resolution_reason   TEXT,                             -- 'condition_cleared' | 'entity_deleted' | 'threshold_change' | NULL
...
[no finding, campaign coverage safe]    → UPDATE status='resolved', set resolved_run/at + resolution_reason
[no finding, campaign coverage unsafe]  → no-op (leave open, don't trust absence)
...
PROCEDURE ingest_run(instance_id, run_data, findings[]):
  ...
  5b. INSERT INTO run_sections for each data section fetched during this run
      (record status='success'|'partial'|'error'|'skipped' and is_complete for each)
      UPDATE runs SET coverage_status based on overall section results

  6. COVERAGE-GATED AUTO-RESOLVE:
     Only auto-resolve issues whose campaign's required sections ALL completed successfully.

     -- Build set of safe campaigns for this run
     safe_campaigns = campaigns where ALL required sections have is_complete=1 in run_sections

     FOR EACH open/regressed issue WHERE campaign_id IN safe_campaigns
       AND NOT IN this run's findings:

       -- Determine resolution reason
       IF entity exists in known_projects/known_users with last_seen_run = this run:
         resolution_reason = 'condition_cleared'   -- entity still exists, condition fixed
       ELIF entity's last_seen_run < this run:
         resolution_reason = 'entity_deleted'      -- entity gone entirely
       ELSE:
         resolution_reason = 'condition_cleared'   -- default

       UPDATE issues SET status='resolved', resolved_run=run_id, resolved_at=now,
                         resolution_reason=?

     -- Issues whose campaign is NOT in safe_campaigns: leave untouched (no false fixes)
```

---

## Email Preview + Send Hooks

### Preview step (`/api/tools/email/preview`)

When generating email previews, the backend already knows which findings/issues each preview message covers. Tag each preview item with the exact issue IDs:

```
FOR EACH preview item:
  1. Resolve the specific issue_ids this message covers
     (from findings for this run + campaign + the entities listed in the message)
  2. Return issue_ids[] alongside each preview item in the response
```

The preview response shape gains a `linkedIssueIds` array per item:
```json
{ "recipientLogin": "john", "email": "john@co.com", "linkedIssueIds": [12, 47, 53], ... }
```

### Send step (`/api/tools/email/send`)

When sending, the frontend passes the `linkedIssueIds` from the preview through to the send request. No fuzzy lookup needed:

```
FOR EACH sent email (with its linkedIssueIds from preview):
  1. INSERT INTO outreach_emails (run_id, campaign_id, recipient_login, ...)  →  get email_id
  2. INSERT INTO outreach_email_issues (email_id, issue_id) for each linkedIssueId
  3. UPDATE issues SET times_emailed = times_emailed + 1, last_emailed_at = now
     WHERE issue_id IN (linkedIssueIds)
```

This is **exact by construction** — the linkage comes from what was actually in the email, not from a runtime guess of "all open issues for this recipient."
...
|---|---|
| `GET /api/tools/outreach-data` | After computing, call `ingest_run()` to persist findings + section statuses |
| `POST /api/tools/email/preview` | Resolve and return `linkedIssueIds[]` per preview item |
| `POST /api/tools/email/send` | Accept `linkedIssueIds` per item; after sending, record emails + exact issue links |
...
6. **Entity granularity matches outreach granularity** — Projects for 10 campaigns, code_envs for 1, scenarios for 3. Scenario campaigns track at the individual scenario level (project_key:scenario_id) so that fixing scenario S1 can be tracked independently of S2 still failing in the same project.
...
9. **Coverage-gated auto-resolution prevents false fixes** — `run_sections` tracks per-section completion, and auto-resolve only runs for campaigns whose required sections all completed successfully. A timed-out or partial run never produces false "resolved" conclusions.

10. **Exact email-to-issue linkage** — Preview generates explicit `linkedIssueIds` per message; send passes them through. No fuzzy recipient+campaign matching that would over-link issues not actually mentioned in the email body.
