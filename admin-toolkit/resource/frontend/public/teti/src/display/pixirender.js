...
export class PixiRender {
    ...
    async init() {
        ...
        this.seekBar = containers["seekBar"];

        Game.particles.initBoard();
        await this.generateTextures();
        this.resize();

        this.app.ticker.add((deltaTime) => this.tick(deltaTime.deltaMS));
    }

    // RENDER CLOCK
    tick(dt) {
        if (!Game.replay.seeking || Game.replay.state != "paused") {
            Game.replay.tick();
            Game.controls.runKeyQueue(Game.stats.time * 1000);
            Game.controls.timer(Game.stats.time * 1000);
        }
        ...
        this.TickSpeedrunUI(Game.zenith.GetFloorLevel(Game.stats.altitude), dt)

        if (Game.gameTimer) this.gameClock(dt);
        if (Game.zenithTimer) Game.zenith.zenithModeTick();
    }

    gameClock(dt) {
        Game.renderer.renderSidebar();
        Game.modes.checkFinished();
    ...
    resize() {
        ...
        this.visuals.createGridGraphics(scaleConsts, icons);
        this.visuals.createTexts(scaleConsts);
        this.generateGrid();
    ...
    buttonGraphics(width) {
        ...
        const isMuted = Game.settings.volume.sfxLevel == 0 && Game.settings.volume.audioLevel == 0;
        const mute = iconframe(isMuted ? this.volumeMutedIcon : this.volumeIcon, 0.16, width * 3 / 20)
        mute.x += width * 0.02;
        mute.y += width * 0.015;
        this.muteButton = mute;
        this.isMuted = isMuted;
        mute.on("pointerdown", () => this.toggleMute());

        const settings = iconframe(this.settingsIcon, 0.18, width * 6 / 20)
        settings.on("pointerdown", () => Game.modals.openModal("settingsPanel"));
        const edit = iconframe(this.editIcon, 0.21, width * 9 / 20)
        edit.on("pointerdown", () => Game.modals.openModal("editMenu"));
        edit.visible = this.editButtonVisible

        return { settings, reset, edit, mute };
    }

    toggleMute() {
        this.isMuted = !this.isMuted;
        if (this.isMuted) {
            this._savedSfx = Game.settings.volume.sfxLevel;
            this._savedAudio = Game.settings.volume.audioLevel;
            Game.settings.volume.sfxLevel = 0;
            Game.settings.volume.audioLevel = 0;
            Game.sounds.setAudioLevel();
            this.muteButton.texture = this.volumeMutedIcon;
        } else {
            Game.settings.volume.sfxLevel = this._savedSfx || 50;
            Game.settings.volume.audioLevel = this._savedAudio || 50;
            Game.sounds.setAudioLevel();
            if (Game.sounds.songs.length > 0 && Game.started) {
                Game.sounds.startSong();
            }
            this.muteButton.texture = this.volumeIcon;
        }
    }
    ...
    async generateTextures() {
        ...
        try {
            ...
            if (texture == null) {
                console.error("Error with texture")
                texture = await PIXI.Assets.load('./assets/skins/tetrio.png');
            }
        } catch (error) {
            console.error(error)
            texture = await PIXI.Assets.load('./assets/skins/tetrio.png');
        ...
        this.volumeIcon = await PIXI.Assets.load('./assets/icons/volume.svg');
        this.volumeMutedIcon = await PIXI.Assets.load('./assets/icons/volume-muted.svg');
    ...
    TickSpeedrunUI(floor, dt) {
        ...
        for (let t = 0; t < 9; t++) {
            ...
            s.inner.alpha = .75 - ((i * (dt / 1000) * 4) % 2) / 10
            let o = 0;
            if (Game.zenith.isHyperspeed) {
                switch (Math.floor((i * (dt / 1000) * 2) % 2)) {
                    case 0:
                        o = -1;
                        break;
                    case 1:
