    ...
    const run = async () => {
      ...
      try {
        ...
        const runModelUpgradeAudit = async () => {
          const auditPath = '/api/tools/model-upgrade-audit/scan?mode=used_available';
          const auditStart = nowMs();
          ...
          log(`GET ${auditPath}`);
          try {
            const url = getBackendUrl(auditPath);
            ...
            while (true) {
              ...
              for (const part of parts) {
                ...
                if (eventType === 'init') {
                  ...
                  log(total > 0 ? `Model audit queued (${total} projects)` : 'Model audit queued, discovering projects');
                  ...
                  if (
                    totalProjects > 0 &&
                    scanned > 0 &&
                    (scanned === totalProjects || lastLoggedScanned < 0 || scanned - lastLoggedScanned >= 50)
                  ) {
                    lastLoggedScanned = scanned;
                    log(
                      `Model audit progress ${scanned}/${totalProjects} refs=${Number(payload.referencesFound || 0)} errors=${Number(payload.errorsCount || 0)}`,
                    );
                  }
        ...
        log(
          'Phase 3 strategy: launch code-envs + project-footprint + connection-health in parallel; model-audit already started in Phase 1; defer dir-tree until Directory page is opened',
        );
